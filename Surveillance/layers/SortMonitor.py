...
class SortActivity(PuzzleActivities):
  """!
  @brief  Class detects the state of the sort process
  """
  ...
  def  __init__(self, imRegions):
    ...
    # Set internal state
    self.x = SortState(regions=[None] * (self.lMax + 1), zoneOcc=[0] * (self.lMax + 1), piecesPlaced=0, totalPieces=0)
    
    # Set output state
    self.z = SortOutput(rgb=None, pcInfo=None, actor='', haveObs=False)
  ...
  def measure(self, y: SortInput):
   ...
   # First run calibration
    ...
    # Searching for picks in Unorganized zone and places in solution zone. 

    if self.x.zoneOcc[self.unorg] == 1 and zoneOcc[self.unorg] == 0:
      # check if piece was removed
      diff = self.x.regions[self.unorg] - regions[self.unorg]
      ...
      if len(pick_props) > 0:
          ...
          print(f"Detected human pick in zone {self.unorg} at ({pick_x:.1f}, {pick_y:.1f})")
          self.z.pcInfo = Piece(pick=np.array([pick_x, pick_y]), place=None, zone=self.unorg, pick_time=time.time(), place_time=None)
          self.z.actor = 'human'
          update[self.unorg] = True
    ...
    for i in range(1, self.numZones + 1):
        if self.x.zoneOcc[i] == 1 and zoneOcc[i] == 0:
            ...
            if len(place_props) > 1:
                ...
                # display a plot of piece place found
                # plt.imshow(clean_mask, cmap='gray')
                # plt.title(f"Zone {i} - Detected Place")
                # plt.show()
                prop = place_props[0]
...
class SortMonitor(PuzzleMonitor):
  ...
  def measure(self, I):
    ...
    actInput = SortInput(sceneState=pstate, image=I)
