...
################################################################################
# Sort Monitor support classes
################################################################################
@dataclass
class StatePuzzleSortActivity(StatePuzzleActivity):
  '''!
  @ingroup  Surveillance
  @brief    Perceiver puzzle scene sort activity structure for solving
            by human. 
  ...
  image    | RGB image.
  zones    | State of pieces in each zone
  '''
  image    : any
...
class SortActivity(PuzzleActivities):
  ...
  def measure(self, y):
    """
    @brief  Compare image signal to empty zones and generate sigal.
    @param[in]  y  SortActivityInput
    """
    
    noCounts = ZoneCounter({x:0 for x in range(self.lMax+1)})
    self.z = StatePuzzleSortActivity(hand = [-1], puzzle = None, zones = noCounts, image=None)
...
class SolveActivity(PuzzleActivities):
  ...
  def measure(self, y):
    ...
    """
    puzzleSeg = self.imRegions * y.segIm
    # print(np.unique(puzzleSeg))
    self.z = StateSolveActivity(0, [-1])
